...
import { ref as ref3, unref as unref3, watch as watch3, computed as computed3, reactive as reactive3, toRaw, inject as inject3, onUpdated as onUpdated2, nextTick as nextTick3, defineComponent as defineComponent2, useSlots, h as h2, Fragment as Fragment2, isRef as isRef3 } from "vue";
...
function useMotionControls(motionProperties, variants = {}, { motionValues, push, stop } = useMotionTransitions()) {
  const _variants = unref3(variants);
  const isAnimating = ref3(false);
  watch3(
    motionValues,
    (newVal) => {
...
function registerEventListeners({ target, state, variants, apply }) {
  const _variants = unref3(variants);
  const hovered = ref3(false);
  const tapped = ref3(false);
  const focused = ref3(false);
  const mutableKeys = computed3(() => {
  ...
  const computedProperties = computed3(() => {
    const result = {};
...
function reactiveTransform(props = {}, enableHardwareAcceleration = true) {
  const state = reactive3({ ...props });
  const transform = ref3("");
  watch3(
    state,
    (newVal) => {
      let result = "";
...
function useMotionProperties(target, defaultValues) {
  const motionProperties = reactive3({});
...
function directive(variants, isPreset = false) {
  const register = (el, binding, node) => {
    ...
    const variantsRef = ref3(variantsObject);
  ...
  return {
    ...
    getSSRProps(binding, node) {
      ...
      bindingInitial = unref3(bindingInitial);
...
function setupMotionComponent(props) {
  const instances = reactive3({});
  const customPresets = inject3(CUSTOM_PRESETS, {});
  const preset = computed3(() => {
...
var MotionComponent = defineComponent2({
  name: "Motion",
  props: {
  ...
  setup(props) {
    ...
    return () => {
      ...
      const node = h2(props.is, void 0, slots);
      setNodeInstance(node, 0, style);
      return node;
    };
  }
});
var MotionGroupComponent = defineComponent2({
  name: "MotionGroup",
  props: {
    ...MotionComponentProps,
    is: {
  ...
  setup(props) {
    ...
    return () => {
      ...
      for (let i = 0; i < nodes.length; i++) {
        const n = nodes[i];
        if (n.type === Fragment2 && Array.isArray(n.children)) {
      ...
      if (props.is) {
        return h2(props.is, void 0, nodes);
      }
      return nodes;
    };
  }
...
function isMotionInstance(obj) {
  const _obj = obj;
  return _obj.apply !== void 0 && typeof _obj.apply === "function" && _obj.set !== void 0 && typeof _obj.set === "function" && _obj.target !== void 0 && isRef3(_obj.target);
}
function useMotions() {
  return motionState;
}
